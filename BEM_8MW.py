...
segments = np.linspace(4.8, R-0.2, 17)

# Chord Length for Radial Position (m)
chords = 2 * np.array([1.63, 1.597, 1.54, 1.481, 1.42,
...
# Profile of Blade for Radial Position
profiles = [0, 0, 5, 46, 19,
            27, 10, 24, 28, 22,
            20, 39, 43, 37, 45,
            44, 35]

profiles = [0, 5, 46, 19, 27,
            10, 24, 28, 22, 20,
            39, 43, 37, 45, 44,
            35, 32]
...
for n, thetap in enumerate(thetaps):
    ...
    # Twist Angle for Radial Position (degree)
    ...
    for n, V0 in enumerate(speeds):
        ...
        for m, r in enumerate(segments):
            ...
            filenumb = f"{profile:02d}"
            fcl, fcd = cld_func("Profile-" + filenumb + "-CLD.csv")
...
# plt.ylim(0, 600)
plt.axhline(8000, color="black", linestyle="--")
plt.show()
...
plt.figure(1, figsize=(6, 6))
for i in range(len(P_final)):
    plt.plot(np.array(list(reversed(P_final)))[i]/1000, np.sum(list(reversed(T_final))[i], 1)/1000)
